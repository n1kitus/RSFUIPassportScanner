...
class ScannerViewController: UIViewController {
    static let queuePrefix = "com.revolver.passportscanner"
    ...
    public var scanningIsEnabled = true {
        didSet {
            captureSessionQueue.async { [weak self] in
                guard let isEnabled = self?.scanningIsEnabled else { return }
                if isEnabled {
    ...
    private let captureSessionQueue = DispatchQueue(label: "\(ScannerViewController.queuePrefix).captureSessionQueue")

    private var captureDevice: AVCaptureDevice?

    private let videoDataOutput = AVCaptureVideoDataOutput()
    private let videoDataOutputQueue = DispatchQueue(label: "\(ScannerViewController.queuePrefix).videoDataOutputQueue")
    ...
    private func showBoundingRects(valid validRects: [CGRect], invalid invalidRects: [CGRect]) {
        ...
        for rect in invalidRects {
            ...
            draw(rect: rect, color: UIColor.red.cgColor)
        }
